...
public class PizzaRestaurant {
	...
	public boolean processLog(String filename) throws CustomerException, PizzaException, LogHandlerException{
		...
		ArrayList<Customer> _customers=LogHandler.populateCustomerDataset(filename);
		
		ArrayList<Pizza> _pizzas=LogHandler.populatePizzaDataset(filename);
		
		customers.addAll(_customers);
		pizzas.addAll(_pizzas);
	...
	public Pizza getPizzaByIndex(int index) throws PizzaException{
		...
		if(index<0||index>pizzas.size()-1){
			throw new PizzaException("Pizzas index out of range.");
		}
